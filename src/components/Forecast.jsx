...
function Forecast({ city, forecast }) {
    ...
    return (
        <div className='container bg-dark text-white mt-4' style={{background: `linear-gradient(rgba(0, 0, 0, 0.00),rgba(0, 0, 0, 0.00)), url('./cloud.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center center'}}>
            <div className='py-2 px-3 px-sm-4'>
                <h5 className='mb-0'>
                    <span>{city}</span>
                    <span className='fw-normal fs-6 ms-1 ms-sm-2'>next seven (7) days forcast</span>
                </h5>
            </div>
                ...
                {forecast.list.splice(0, 7).map((item, index) => (
                    <AccordionItem className='rounded border border-light my-2 overflow-hidden' key={index}>
                        <AccordionItemHeading>
                            <AccordionItemButton>
                                <div className="py-2 px-3" style={{ background: 'hsla(0, 0%, 0%, 25%)' }}>
